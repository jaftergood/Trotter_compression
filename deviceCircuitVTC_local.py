...
if __name__ == "__main__":
    ...
    def SwapTestExecutor(circuits, backend, shots, filter):
        ...
        zero_noise_values = []
        if isinstance(backend, QasmSimulator): # exact_sim
    ...
    def LoschmidtEchoExecutor(circuits, backend, shots, filter):
        ...
        folded_circuits_transpiled = transpile(folded_circuits, backend, initial_layout=qb_map)

        job = execute(
            experiments=folded_circuits_transpiled,
            backend=backend,
            optimization_level=0,
            shots=shots
        )
        ...
        X = [[] for _ in range(L)]
        chk = sum([int(c[i]) for i in range(len(c))])
        for i in range(len(X)):
            j = 0
            while j < 2**L:
                if (c[i] == all_possible_states[j][i]) and (chk == sum([int(all_possible_states[j][k]) for k in range(len(all_possible_states[j]))])):
                    X[i].append(all_possible_states[j])
                j += 1
        ...
        for counts in all_counts:
            ...
            for i in range(len(X)):
                for j in range(len(X[i])):
                    if counts.get(''.join(X[i][j])) is None:
                        placeholder.append(0)
                    else:
                        placeholder.append(counts.get(''.join(X[i][j]))/total_allowed_shots)
            expectation_values.append(sum(placeholder)/L)
    ...
    def LoschmidtEchoCircuit(params, U_v, U_trot, init, p):
        ...
        circ = init.compose(U_v).compose(U_trot).compose(U_v_prime.inverse())
        circ.measure_all()
        return circ
    ...
    def VTC(tf, dt, p, init, backend, shots, filter):
        ...
        if (os.path.exists(f'./results_{L}/VTD_params_{tf}_{L}_{p}_{dt}_{shots}_loc.csv')):
            VTCParamList = pd.read_csv(f'./results_{L}/VTD_params_{tf}_{L}_{p}_{dt}_{shots}_loc.csv', index_col=0)
            VTCStepList = pd.read_csv(f'./results_{L}/VTD_results_{tf}_{L}_{p}_{dt}_{shots}_loc.csv', index_col=0)
        ...
        VTCStepList.loc[ts[-1]+(dt*p)] = np.array(SimulateAndReorder(init.compose(AnsatzCircuit(res, p))))
        ts = VTCParamList.index

        # VTCParamList = pd.DataFrame(np.array(VTCParamList), index=np.array(TimeStep))
        # VTCStepList = pd.DataFrame(np.array(VTCStepList), index=np.array(TimeStep))

        VTCParamList.to_csv(f'./results_{L}/VTD_params_{tf}_{L}_{p}_{dt}_{shots}_loc.csv')
        VTCStepList.to_csv(f'./results_{L}/VTD_results_{tf}_{L}_{p}_{dt}_{shots}_loc.csv')

        if (ts[-1] >= tf):
            return
        else:
            os.system(f'python deviceCircuitVTC_local.py -L {L} -p {p} -t {tf} -d {dt} -s {shots}')
    ...
    # noise_model = NoiseModel.from_backend(device_sim)
    ...
    t_qc = transpile(meas_calibs, device_sim, initial_layout=qb_map) # real_device
    qobj = assemble(t_qc, shots=32000)
    cal_results = device_sim.run(qobj, shots=32000).result()
    ...
    all_possible_states = []
    for i in range(2**L):
        all_possible_states.append(format(i, f'0{L}b'))


    nt = int(np.ceil(tf / (dt * p)))
    f = open(f'./results_{L}/logging.txt', 'a')
    sys.stdout = f
    VTC(tf, dt, p, init, exact_sim, shots, meas_fitter.filter)
    f.close()